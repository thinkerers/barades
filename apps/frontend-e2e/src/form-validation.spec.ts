...
async function openSandboxPollForm(
  page: Page,
  createPollSandbox: (options?: Record<string, unknown>) => Promise<PollSandboxContext>,
  testInfo: TestInfo,
...
test.describe('Form Validation', () => {
  ...
  test.describe('Poll Creation Form', () => {
    test('should require poll title', async ({ authenticatedPage: page, createPollSandbox }, testInfo) => {
    ...
    test('should require minimum 2 dates', async ({ authenticatedPage: page, createPollSandbox }, testInfo) => {
      await openSandboxPollForm(page, createPollSandbox, testInfo);
    ...
    test('should reject empty poll title', async ({ authenticatedPage: page, createPollSandbox }, testInfo) => {
      await openSandboxPollForm(page, createPollSandbox, testInfo);
    ...
    test('should reject poll title over maximum length', async ({ authenticatedPage: page, createPollSandbox }, testInfo) => {
    ...
    test('should prevent duplicate dates', async ({ authenticatedPage: page, createPollSandbox }, testInfo) => {
      await openSandboxPollForm(page, createPollSandbox, testInfo);
    ...
    test('should prevent past dates', async ({ authenticatedPage: page, createPollSandbox }, testInfo) => {
      await openSandboxPollForm(page, createPollSandbox, testInfo);
      ...
      const errorOrEmpty = page.getByText(/date.*passé|past date|date.*future/i)
        .or(page.locator('.date-chip, .date-tag'));
      await expect(errorOrEmpty.first()).toBeVisible({ timeout: 3000 });
    });

    test('should allow removing added dates', async ({ authenticatedPage: page, createPollSandbox }, testInfo) => {
