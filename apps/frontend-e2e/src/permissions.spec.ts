...
test.describe('Permissions and Authorization', () => {
  ...
  test.describe('Unauthenticated Access', () => {
    ...
    test('should show login page for unauthenticated API calls', async ({ page }) => {
      ...
      const response = await page.request.get('http://localhost:3000/api/groups');
  ...
  test.describe('Private Groups Access', () => {
    test('should hide private groups from non-members', async ({ page, loginAs }) => {
    ...
    test('should block direct URL access to private groups for non-members', async ({ page, loginAs }) => {
    ...
    test('should prevent API access to private group details for non-members', async ({ page, loginAs }) => {
      ...
      const privateGroup = groups.find((g: { name: string }) => g.name === 'Elite Strategy Players');
  ...
  test.describe('Poll Permissions', () => {
    test('should not show create poll button for non-members', async ({ page, loginAs }) => {
      ...
      const brusselsCard2 = page.locator('.group-card', { hasText: 'Brussels Adventurers Guild' });
      await brusselsCard2.getByRole('link', { name: 'Voir les détails' }).click();
    ...
    test('should return 403 when non-member tries to vote via API', async ({ page, loginAs }) => {
      ...
      const response = await page.request.patch('http://localhost:3000/api/polls/fake-poll-id/vote', {
        headers: { 'Authorization': `Bearer ${token}` },
  ...
  test.describe('Role-Based Access', () => {
    ...
    test('should prevent regular members from managing group', async ({ page, loginAs }) => {
      ...
      const casualCard = page.locator('.group-card', { hasText: 'Casual Board Gamers' });
      await casualCard.getByRole('link', { name: 'Voir les détails' }).click();
      
      // Should NOT see management options
      const editButton = page.locator('[aria-label*="edit"], [aria-label*="settings"], button:has-text("Éditer")');
      await expect(editButton).toBeHidden();
    });
  });

  test.describe('Data Leakage Prevention', () => {
    test('should not expose private group IDs in public listings', async ({ page, loginAs }) => {
      await loginAs(page, 'bob_boardgamer');
      await page.goto('/groups');
  ...
  test.describe('Token and Session Management', () => {
    ...
    test('should clear sensitive data on logout', async ({ authenticatedPage: page, logout }) => {
      // Verify token exists
