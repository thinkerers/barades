...
test.describe('Error Handling', () => {
  ...
  test.describe('Network Failures', () => {
    test('should show error message when API is unreachable', async ({ authenticatedPage: page }) => {
      ...
      await expect(page.getByText(/erreur|error|impossible/i)).toBeVisible({ timeout: 10000 });
    });
    ...
    test('should handle groups API timeout gracefully', async ({ authenticatedPage: page }) => {
      ...
      await expect(page.getByText(/chargement|loading/i).or(page.locator('.spinner, .loading'))).toBeVisible({ timeout: 5000 });
      
      // After timeout, should show error
      await expect(page.getByText(/erreur|timeout|délai dépassé/i)).toBeVisible({ timeout: 35000 });
    });
    ...
    test('should allow retry after network error', async ({ authenticatedPage: page }) => {
      ...
      await expect(page.getByText(/erreur|error/i)).toBeVisible({ timeout: 10000 });
  ...
  test.describe('API Errors', () => {
    test('should handle 500 Internal Server Error', async ({ authenticatedPage: page }) => {
      ...
      await expect(page.getByText(/erreur serveur|server error|problème technique/i)).toBeVisible({ timeout: 10000 });
      
      // Should NOT show technical details to user
      await expect(page.getByText(/stack trace|line \d+|internal/i)).toBeHidden();
    });
    ...
    test('should handle 404 Not Found for missing group', async ({ authenticatedPage: page }) => {
      ...
      await expect(page.getByText(/groupe.*introuvable|not found|n'existe pas/i)).toBeVisible({ timeout: 10000 });
    });
    ...
    test('should handle 503 Service Unavailable', async ({ authenticatedPage: page }) => {
      ...
      await expect(page.getByText(/maintenance|indisponible|unavailable/i)).toBeVisible({ timeout: 10000 });
    });
  });
  ...
  test.describe('Slow Network', () => {
    test('should show loading state during slow API calls', async ({ authenticatedPage: page }) => {
      ...
      await expect(page.getByText(/chargement|loading/i).or(page.locator('.spinner, .loading, [data-testid="loading"]'))).toBeVisible();
  ...
  test.describe('Partial Failures', () => {
    test('should handle mixed success/failure when loading groups', async ({ authenticatedPage: page }) => {
      let requestCount = 0;
  ...
  test.describe('Offline Mode', () => {
    test('should detect and notify when going offline', async ({ authenticatedPage: page }) => {
      ...
      await expect(page.getByText(/hors ligne|offline|connexion/i)).toBeVisible({ timeout: 10000 });
